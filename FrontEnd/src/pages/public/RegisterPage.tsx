...
const RegisterPage = () => {
  ...
  const registerSchema = Yup.object().shape({
    ...
    email: Yup.string().required('Email is required').email('Input text must have a valid email.'),
    password: Yup.string().required('Password is required.').min(8, 'Password must be at least 8 charcters'),
  ...
  const onSubmitRegisterForm = async (data: IRegisterDto) => {
    try {
      ...
      if (status === 400 || status === 409) {
        ...
        toast.error('An error occurred. Please contact to Admin')
      }
    }
  }
    ...
    <div className='pageTemplate1'>
      ...
      <div className='max-sm:hidden flex-1 min-h-[600px] h-4/5 bg-gradient-to-tr from-[#DAC6FB] via-amber-400 to-[#AAC1F6] flex flex-col justify-center items-center rounded-l-2xl'>
        <div className='h-3/5 p-6 rounded-2xl flex flex-col gap-8 justify-center items-start bg-white bg-opacity-20 border border-[#ffffff55] relative'>
          <h1 className='text-6xl font-bold text-[#754eb4]'>Dev Power</h1>
      ...
      <form onSubmit={handleSubmit(onSubmitRegisterForm)}
        ...
        <div className='px-4 mt-2 mb-6 w-9/12 flex gap-2'>
          ...
          <Link to={PATH_PUBLIC.login} className='text-[#754eb4] border border-[#754eb4] hover:shadow-[0_0_5px_2px_#754eb44c] px-3 rounded-2xl'>
            Log in
          </Link>
        </div>
        <div className='flex justify-center items-center gap-4 mt-6'>
          <Button variant='secondary' type='button' label='Reset' onClick={() => reset()}></Button>
          <Button variant='primary' type='submit' label='Register' onClick={() => { }} loading={loading}></Button>
